...
object TestUtil {
  ...
  def getBaseArticle: Article = {
    Article(1, 1, "http://example.com", Some("http://example.com/foo.jpg"), "title", Some("description"), Seq("tag"), "site_name", 1, 1, 1, DateTime.now)
  }

  def getBaseArticleMapper: ArticleMapper = {
    ArticleMapper(1, 1, "http://example.com", Some("http://example.com/foo.jpg"), "title", Some("description"), Some("tag"), 1, 1, 1, DateTime.now)
  }

  def getBaseSite: Site = {
    Site(1, "site_name", "http://", Some("http://example.com/1.jpg"), 1, 1, 1, Seq(getBaseArticle))
  }
